...
class Program
{
    ...
    static async Task RerunRoute(string sourceConn)
    {
        ...
        logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone), "RerunRoute", "Start", "RerunRoute migration started", null, null, null, null));

        try
        {
            string sqlPath = Path.Combine(Directory.GetCurrentDirectory(), "SQL", "RerunRoute.sql");
            ...
            logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone), "RerunRoute", "End", "RerunRoute migration ended", null, null, null, null));
        }
        catch (Exception ex)
        {
            var ts = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone);
