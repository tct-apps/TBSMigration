...
class Program
{
    ...
    static async Task AdhocSchedule(string sourceConn)
    {
        ...
        {
            // Load SQL
            string sqlPath = Path.Combine(Directory.GetCurrentDirectory(), "SQL", "AdhocSchedule.sql");
                ...
                foreach (var batch in allTrips.Chunk(100))
                {
                    ...
                    try
                    {
                        ...
                        logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone),
                                  "Trip", "Insert", $"Date: {tripDate:yyyy-MM-dd} Batch Records: {batchSize}",
                                  requestXml, responseXml, customData, isSuccess));
                    }
                    catch (Exception ex)
                    {
                        var ts = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone);
                        LogMigrationException.Error(ts, "Trip", "Insert", requestXml, responseXml,
                    ...
                    try
                    {
                        ...
                        logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone),
                                  "Trip", "Update", $"Date: {tripDate:yyyy-MM-dd} Batch Records: {batchSize}",
                                  null, null, $"{tripDate:yyyy-MM-dd}", isSuccess));
                    }
                    catch (Exception ex)
                    {
                        var ts = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone);
                        LogMigrationException.Error(ts, "Trip", "Update", requestXml, responseXml,
