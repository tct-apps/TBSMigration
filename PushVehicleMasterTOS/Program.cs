...
class Program
{
    ...
    static async Task Vehicle(string sourceConn)
    {
        ...
        logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone), "Vehicle", "Start", "Vehicle migration started", null, null, null, null));

        try
        {
            string sqlPath = Path.Combine(Directory.GetCurrentDirectory(), "SQL", "Vehicle.sql");
            ...
            logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone), "Vehicle", "End", "Vehicle migration ended", null, null, null, null));
        }
        catch (Exception ex)
        {
            var ts = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone);
