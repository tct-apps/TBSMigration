...
class Program
{
    ...
    static async Task BusOperator(string sourceConn)
    {
        ...
        logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone), "BusOperator", "Start", "BusOperator migration started", null, null, null, null));

        try
        {
            string sqlPath = Path.Combine(Directory.GetCurrentDirectory(), "SQL", "BusOperator.sql");
            ...
            string soapAction = soapActionBase + Constant.ApiUrlKey.BusOperatorInsert;
            ...
            logs.Add((TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone), "BusOperator", "End", "BusOperator migration ended", null, null, null, null));
        }
        catch (Exception ex)
        {
            var ts = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, malaysiaTimeZone);
